      ...
      <div v-if="!showForm" class="space-y-4">
        ...
        <div v-else class="grid gap-4">
          ...
          >
            ...
            <div
              v-else
              class="flex-shrink-0 w-32 h-32 bg-gray-100 rounded-lg flex items-center justify-center"
            >
            ...
            <div class="flex-grow">
              ...
              <div class="text-sm text-text-secondary">
                <span v-if="project.category" class="mr-4"
                  >📁 {{ project.category }}</span
                >
                <span v-if="project.status" class="mr-4"
                  >⚡ {{ project.status }}</span
                >
              </div>
            </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                >
                  ...
                  <button
                    type="button"
                    @click="removeImage(i)"
                    class="absolute top-2 right-2 bg-white/80 rounded-full p-1 text-red-600"
                  >
                    ✕
                  </button>
                </div>
                ...
                >
                  <img
                    :src="
                      Array.isArray(form.image_urls) && form.image_urls.length
                        ? form.image_urls[0]
                        : form.image_url || ''
                    "
                    alt="Preview"
...
const handleSubmit = async () => {
  ...
  console.debug("[admin] handleSubmit start", {
  ...
  try {
    ...
    if (selectedFiles.value.length > 0) {
      ...
      for (const original of selectedFiles.value) {
        ...
        const safeFile = new File([original], safeName, {
          type: original.type,
        });

        const { data: uploadData, error: uploadError } =
          await db.storage.uploadProjectImage(safeFile);
      ...
      if (
        oldImageUrl.value &&
        uploadedUrls[0] &&
        oldImageUrl.value !== uploadedUrls[0]
      ) {
    ...
    if (editingProject.value) {
      ...
      if (res?.error) {
        ...
        if (
          ...
          (res.error as any).message?.includes(
...
const handleDelete = async () => {
  ...
  try {
    ...
    if (
      deleteConfirm.value.image_urls &&
      Array.isArray(deleteConfirm.value.image_urls)
    ) {
      // remove each image path/url
      for (const img of deleteConfirm.value.image_urls) {
        try {
          await db.storage.deleteProjectImage(img);
        } catch (e) {
          /* ignore */
        }
      }
